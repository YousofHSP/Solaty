...
namespace WebClient.Components.Pages.Sessions;

public partial class SessionsIndex : ComponentBase
{
    ...
    {
        var result = await BaseService.Post<IndexDto, IndexResDto<ApiTokenResDto>>("v1/User/GetTokens", _indexDto);
    ...
    private async Task Disable(int id)
    {
        try
        {
            ...
            var result = await BaseService.Post("v1/User/DisableTokens", dto);
            if (result)
            {
